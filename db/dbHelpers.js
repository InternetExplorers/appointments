...
const generateFakeBusinessData = () => {
  for (let j = 0; j < 100; j + 1) {
    ...
    const openTimes = `${openingHours[Math.floor(Math.random() * Math.floor(openingHours.length - 1))]}:${minutes[Math.floor(Math.random() * Math.floor(1))]}:00`;
    const closeTimes = `${closingHours[Math.floor(Math.random() * Math.floor(openingHours.length - 1))]}:${minutes[Math.floor(Math.random() * Math.floor(1))]}00`;
    const guestMax = Math.floor(Math.random() * Math.floor(50));
    const sqlString = 'INSERT INTO appointments_log (name, address, city, state, zip, opens, closes, guest_max) VALUES(?, ?, ?, ?, ?, ?, ?, ?)';
